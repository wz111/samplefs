...
//                        inode->__i_nlink++;
			inc_nlink(inode);
                        break;
		case S_IFLNK:
			printk(KERN_INFO "samplefs: GFP_USER = 0x%x\n", GFP_USER);
			printk(KERN_INFO "samplefs: GFP_HIGHUSER = 0x%x\n", GFP_HIGHUSER);
			printk(KERN_INFO "samplefs: __GFP_HIGHMEM = 0x%x\n", __GFP_HIGHMEM);
                	inode->i_op = &page_symlink_inode_operations;
			inode_nohighmem(inode);
			printk(KERN_INFO "samplefs: BUG_ON = 0x%x\n", mapping_gfp_mask(inode->i_mapping) & __GFP_HIGHMEM);
			break;
                }
        }
        return inode;
